...
from message_broker.handlers.base_handler import (
    BaseMessageHandler, ChannelCallbackHandler, ChannelHandler, ConnectionMethod, ConnectionHandler, ConsumerCallbackHandler, ConsumerHandler,
    ConnectionCallbackHandler, ExchangeCallbackHandler, ExchangeHandler, QueueCallbackHandler, QueueHandler
)
...
class Exchange(ExchangeHandler):
    ...
    def setup_exchange(self, channel: PikaChannel):
        # TODO: Raise exception if callback is not present
        self._logger.info(
            f'Declaring exchange: {self.name}')
...
class Queue(QueueHandler):
    ...
    def __init__(self, name: str, routing_key: str, prefetch_count: int, logger: LoggingHandler):
        self.name = name
    ...
    def bind_queue(self, exchange: ExchangeHandler, channel: PikaChannel):
        self._logger.info(f'Binding {exchange.name} to {self.name} with {self._routing_key}')
        channel.queue_bind(
            self.name,
            exchange.name,
    ...
    def set_qos(self, channel: PikaChannel):
        ...
        channel.basic_qos(
            ...
            callback=self._callback.on_qos_ok
            )
...
class ConsumerCallback(ConsumerCallbackHandler):
    ...
    def on_consumer_cancelled(self, method_frame):
        self._logger.info(f'Consumer was cancelled remotely, shutting down: {method_frame}')
        ch = self.consumer.channel.get_channel()
        if ch:
            ch.close()
    ...
    def on_cancel_ok(self, *args, **kwargs):
        self.consumer.consuming = False
        self._logger.info(f'Acknowledged the cancellation of the consumer: {self.consumer.consumer_tag}')
...
class Consumer(ConsumerHandler):
    ...
    def start_consuming(self):
        ...
        self.consumer_tag = self._channel.basic_consume(
            ...
            callback=self._msg_callback.on_message,
            auto_ack=True
            )
